...
- Core IMG value metadata 和 payload diagnostic 投影被拆到独立 helper，
  `ResourceInspectionService` 继续负责 inspection tree、identity 和 link
  组合，不直接承载所有 Canvas / RawData / Video / Sound / Lua / text /
  geometry value family 的字段 switch。
