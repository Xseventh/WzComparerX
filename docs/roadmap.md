...
- Node tree view. Started with a path-based load command bound to Core
  ...
  IMG is suppressed at the command-state layer. Empty top-level split-package
  stubs now expand into linked sibling package nodes when the package workspace
  layout is present.
