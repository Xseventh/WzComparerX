...
- Canvas preview. Started with lazy Canvas value preview through Core using
  ...
  `ARGB8888` (`2`), `DXT3` (`1026`), and `DXT5` (`2050`) decoder slices.
...
- Direct-zlib Canvas preview for formats `1` / `2` was enough for M4 closeout.
  M5 has since added `257`, `513`, `1026`, and `2050`; PNG export, MapRender,
  search, and compare remain later milestones.
...
- Direct-zlib Canvas preview formats `1` / `2` were accepted as the M4 image
  preview slice; M5 extends this to `257`, `513`, `1026`, and `2050`, while
  PNG export stays later.
...
| Canvas/image decode | Multiple pixel formats, display, PNG save, raw export | Direct-zlib viewer slices for `1`/`257`/`513`/`2`/`1026`/`2050`; raw export remains the initial direct-zlib Canvas byte slice | ~20-25% | M8 |
