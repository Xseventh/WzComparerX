...
- The path row has native file and folder pickers. File picking opens WZ
  packages and synthetic fixtures. Folder picking scans `.wz` headers into a
  folder inspection tree; selecting a package node enables `Open Package`.
  Opening a new file or package clears the previous IMG selector before loading.
