...
- Recorded the WC reference files for MS/MN container directory inspection and
  the current directory-only boundary.
...
- `.ms` / `.mn` v2/Snow and v4/ChaCha20 container directory tables are the
  accepted M5 slice.
- `.ms` / `.mn` image payload extraction remains deferred until compare/search
  or direct image browsing needs those payloads as WZ IMG inspections.
