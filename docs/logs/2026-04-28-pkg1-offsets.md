# 2026-04-28 - PKG1 Version And Offset Inspection

## Summary

- Added PKG1 hash-version calculation and encrypted-version matching.
- Added PKG1 entry offset calculation for directory inspection entries.
- Updated text and JSON inspection output with `wzVersion`, `hashVersion`, and
...
```bash
/usr/local/share/dotnet/dotnet run --project src/WzComparerX.Cli --no-build -- inspect --key none path/to/Data/Base/Base.wz
```
