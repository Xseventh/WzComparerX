...
| UOL / link strings | Resolve links where possible; display unresolved links | Link-like values now carry `Identity.LinkedTarget`; `inspect --debug` can populate `Identity.ResolvedLinkedTarget` for local `_inlink`, relative UOL, and logical `source` / `_outlink` / `link` targets with `.img` segments; optional Core GMS smoke validates `Map1_000.wz/100000000.img` `miniMap/_outlink`; Canvas preview uses the same resolver and unresolved Canvas preview links emit `wcx.viewer.canvas.linkUnresolved` | Expand real-client link smoke and add diagnostics for non-viewer unresolved targets only when consumers need them | P0 | Core link fixtures, App preview tests, optional local Map/UI smoke |
...
| Mob / Npc | Optional Core smoke validates representative package roots can be inspected without parser errors | Add representative IMG/property smoke notes |
