...
| Item / Character / Skill | Optional Core smoke validates representative package roots can be inspected without parser errors when local GMS data is available; Character smoke validates `Character_000.wz/00002000.img` `_outlink` identity resolution and Canvas preview through `Character/_Canvas/_Canvas_000.wz` | Add representative `inspect --debug` notes for remaining Item/Skill IMG value families |
| String | Optional Core smoke validates representative package root can be inspected without parser errors | Use as early StringLinker/Search input after M5 identity stabilizes |
...
| Effect | Base link path coverage; optional Core smoke validates package root can be inspected without parser errors | Add Canvas/link smoke if suitable sample is found |
