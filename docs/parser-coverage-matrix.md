...
| PKG1 package groups | `Name.ini` / `LastWzIndex` plus numbered shard merge | Implemented in Core package group layer; `.ini`-declared missing or invalid shards emit `wcx.package.group.shardMissing` / `wcx.package.group.shardInvalid` warnings | Add real-client smoke notes for any package group warnings found locally | P0 | Core temp-fixture tests plus local `Map1.wz` smoke |
