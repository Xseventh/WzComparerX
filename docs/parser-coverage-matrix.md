...
| List.wz / optional containers | WC supports older/newer container helpers; `.ms` and `.mn` both flow through `Wz_Structure.LoadMsFile`, which tries the Snow reader before the ChaCha20 reader | `.ms` and `.mn` v2/Snow and v4/ChaCha20 container directory tables inspect through Core; folder inspection now lists `.ms`/`.mn` packages alongside `.wz`; synthetic `.mn` coverage locks the shared loader path; local GMS `Data/Packs/*.ms` smoke covers 10 Mob/Skill pack files; selecting an `.ms`/`.mn` image reports stable `wcx.package.ms.imageUnsupported`; unsupported shapes report `wcx.package.ms.directoryUnsupported`. Local GMS scan found no `List.wz` and no `.mn` files | Support remains required for older-client compatibility. `.ms`/`.mn` image payload extraction and `List.wz` still need parser slices from WC reference behavior or older-client samples instead of assuming the current GMS layout is complete | P1 | Synthetic `.ms` v2/v4 fixtures, synthetic `.mn` fixture, optional local GMS `.ms` smoke, WC reference review, future older-client smoke samples |
...
1. Freeze the current `.ms` / `.mn` slice as directory-only until compare/search
   needs direct image payload extraction; the next parser slice is entry payload
   extraction as WZ IMG inspection, not more container-table projection.
2. Define the `List.wz` first parser slice from `Wz_Crypto.LoadListWz`: decode
