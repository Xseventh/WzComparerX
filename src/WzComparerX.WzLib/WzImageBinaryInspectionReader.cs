...
internal sealed class WzImageBinaryInspectionReader
{
        ...
        long imageEndOffset)
    {
        ...
        {
            properties = ReadPropertyEntries(stream, imageBaseOffset, imageEndOffset, depth: 0, parentPath: string.Empty, out propertyCount);
        }
        else if (maxPropertyDepth > 0)
        {
            var objectInspection = ReadTopLevelObjectValue(stream, imageBaseOffset, imageEndOffset, objectType);
